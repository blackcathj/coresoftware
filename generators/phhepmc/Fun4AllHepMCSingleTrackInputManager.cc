#include "Fun4AllHepMCSingleTrackInputManager.h"
...
Fun4AllHepMCSingleTrackInputManager::Fun4AllHepMCSingleTrackInputManager(const string &name, const string &nodename, const string &topnodename)
...
Fun4AllHepMCSingleTrackInputManager::~Fun4AllHepMCSingleTrackInputManager()
{
  fileclose();
...
int Fun4AllHepMCSingleTrackInputManager::fileopen(const string &filenam)
{
  if (!mySyncManager)
  {
...
int Fun4AllHepMCSingleTrackInputManager::run(const int nevents)
{
  ...
  while (true)
  {
    if (!isopen)
    {
...

      {
        if (Verbosity() > 0)
        {
          cout << "Fun4AllHepMCSingleTrackInputManager::run::" << Name() << ": No Input file open" << endl;
        }
        return -1;
      }
      else
      {
        if (OpenNextFile())
        {
          cout << "Fun4AllHepMCSingleTrackInputManager::run::" << Name() << ": No Input file from filelist opened" << endl;
          return -1;
        }
      }
    }

    // check whether and event is buffered already.

    if (!evt)
    {
      if (Verbosity())
      {
        cout << "Fun4AllHepMCSingleTrackInputManager::run::" << Name() << ": loada a new HepMC event" << endl;
      }
      ...
      if (!evt)
      {
        if (Verbosity() > 1)
        {
          cout << "Fun4AllHepMCSingleTrackInputManager::run::" << Name()
      ...
      continue;  // try the cycle again
    }
    else if (currentParticle == evt->particles_end())
    {
      if (Verbosity())
      {
    ...
    else if (not IsStateFinal(*currentParticle))
    {
      if (Verbosity())
      {
    ...
    else
    {
      ...
      // load it to the DST tree
      mySyncManager->CurrentEvent(events_total);
      if (Verbosity() > 0)
      {
        cout << "Fun4AllHepMCSingleTrackInputManager::run::" << Name()
             << ": single track from hepmc evt no: " << evt->event_number() << " loaded as event " << events_total << endl;
      }
      ...
      if (ievt != hepmc_helper.get_geneventmap()->end())
      {
        ...
        ievt->second->addEvent(single_tr_evt);
      }
      else
        hepmc_helper.insert_event(single_tr_evt);
...
int Fun4AllHepMCSingleTrackInputManager::fileclose()
{
  if (!isopen)
  {
...
void Fun4AllHepMCSingleTrackInputManager::Print(const string &what) const
{
  Fun4AllInputManager::Print(what);
  return;
}

int Fun4AllHepMCSingleTrackInputManager::OpenNextFile()
{
...
int Fun4AllHepMCSingleTrackInputManager::PushBackEvents(const int i)
{
  ...
  if (i > 0)
  {
    ...
    cout << PHWHERE << Name()
         << " Fun4AllHepMCSingleTrackInputManager cannot pop back events into file"
         << endl;
    return -1;
  }
  if (!isopen)
...
HepMC::GenEvent *
Fun4AllHepMCSingleTrackInputManager::ConvertFromOscar()
{
  delete evt;
  evt = NULL;
